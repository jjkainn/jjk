...
//				$streamtype = "0";
			...
			$SE = new SocketEntity();
			if($streamtype == 1){
				$ret =$SE->SendSetParam(setVideoParam($resolution,$framerate,$ratecontrol,$bitrate,$qp,$iframerate,$encode,$streamtype));
			}else{
	...
	function setRate(){
		if(isset($_POST['rate'],$_POST['videostd'],$_POST['streamtype'])){
			...
			if($streamtype == 1){
				...
				$getvalue	= GetVideoParam($streamtype);//0525
			}else{
				$getvalue	= GetVideoParam($streamtype);
			}
		    $videostd		= $getvalue["videostd"];
...
//			}
	...
	function mainIPNAME(){
		if(isset($_POST['serverIP'])){
			$newpath = $_POST['serverIP'];
			$streamtype = $_POST['streamtype'];
			$newName = $_POST['streamName'];
			// print_r($streamtype);die;
			// print_r($streamtype);
			// print_r($streamName);die;
			/*$chan = getMediaStreamType();
			$chanNo = $chan['LiveStreamType'];*/
			$chanNo = 0;
			$getvalue =&GetMediaPara('RTMP',$chanNo,$streamtype);
			...
			$SE = new SocketEntity();
			// print_r($used);die;
			$ret =$SE->SendSetParam(ChangeMedia($media,$newpath,$newName,$used,$streamtype));
...
//	}
	...
	function setorchName(){
		if(isset($_POST['orchDevName'])){
			...
			$ret = GetDeviceID(); 
	        if($ret['id']=="10"){
	        	$orchDevName = iconv('UTF-8','GB2312//IGNORE',$orchDevName);
	        }
