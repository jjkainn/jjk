...
<?php
	//常用参数
		...
		//获取4G信号强度
			...
			if($netpara["rsrqsignal"] < 2 || $netpara["rsrqsignal"] == 99)
			{
				$netpara["rsrqsignal"] = 0;
			}
			else if($netpara["rsrqsignal"] >= 2 && $netpara["rsrqsignal"] < 7)
			{
				$netpara["rsrqsignal"] = 1;
			}
			else if($netpara["rsrqsignal"] >= 7 && $netpara["rsrqsignal"] < 12)
			{
				$netpara["rsrqsignal"] = 2;
			}
			else if($netpara["rsrqsignal"] >= 12 && $netpara["rsrqsignal"] < 17)
			{
				$netpara["rsrqsignal"] = 3;
			}
			else if($netpara["rsrqsignal"] >= 17 && $netpara["rsrqsignal"] < 22)
			{
				$netpara["rsrqsignal"] = 4;
			}
			else if($netpara["rsrqsignal"] >= 22)
			{
				$netpara["rsrqsignal"] = 5;
			}else{
				$netpara["rsrqsignal"] = 0;
			}
			echo("<script>$('#signalStrength').val(\"{$netpara['rsrqsignal']}\");</script>");
...
<script>
	...
	$("#loginlist").click(function(){
		window.location.href="Channeldisplay.php";
	})
</script>
<?php
	$ret = GetDeviceID(); 
