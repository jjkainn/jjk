								...
								<tr><div>
									...
									<td>
										<select class="form-control" name="mainResolution" id="mainResolution" onchange="setmainResolution(value)">
											...
											<option class="form-control" value=2>360P</option>
										</select>
									</td>
									<script>
								...
								<tr><div>
									...
									<script>
										function setMainServerIP(serverIP){
											...
											RTMPret = /^(?=^.{3,255}$)(rtmp?:\/\/)?(pubsec\.)?[a-zA-Z0-9][-a-zA-Z0-9]{0,62}(\.[a-zA-Z0-9][-a-zA-Z0-9]{0,62})+(:\d+)*(\/\w+\.\w+)*$/;
											...
											var Length = serverIP.indexOf('v');

											var data=serverIP.substring(0,Length+1);

											if (RTMPret.test(data)) {
													var port = ":1935";
													var serverIP = (insert_iten(serverIP,port,Length));
											...
											function insert_item(str,item,index){//js字符串插入方法
												...
												function insert_iten(str,item,index){//js字符串插入方法
...
//			echo("<script>alert(123)</script>");
	    ...
	    if($ret['id']=="10"){
	    	...
	    	$orchDevName =  iconv('GB2312','UTF-8',$orchDevName);
	    }
